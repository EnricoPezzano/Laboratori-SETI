 ...
 * Copyright (C) 2016-2019 by Giovanni Chiola <chiolag@acm.org>
 *
...
/*** TO BE DONE 3.0 START ***/
	...
	pthread_mutex_lock(&cookie_mutex);
			...
			CurUID++; //incremento CurUID

			retval = CurUID%MAX_COOKIES; //setto retval

			UserTracker[retval] = 0; // resetto UserTracker[retval]
...
/*** TO BE DONE 3.0 START ***/
		...
		UserTracker[myUID] = UserTracker[myUID] + 1; //Incremento UserTracker[myUID]

		newcount = UserTracker[myUID]; //Lo setto in newcount per restituirlo
...
/*** TO BE DONE 3.0 START ***/
		...
		if(sendfile(client_fd,fd,NULL,file_size) == -1){
			close(fd);
			fail_errno("Could not copy on client_fd");
		}
		close(fd);
