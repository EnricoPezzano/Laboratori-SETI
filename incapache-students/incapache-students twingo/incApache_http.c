...
/*** TO BE DONE 4.0 START ***/
	if(gmtime_r(&now_t,&now_tm)==NULL)
		printf("the result cannot be represented");
...
/*** TO BE DONE 4.0 START ***/
	sprintf(http_header+strlen(http_header),"\r\nSet-Cookie: UserID=%d; Expires=Fri, 31 Dec 2021 23:59:59 GMT",cookie);
/*** TO BE DONE 4.0 END ***/
...
/*** TO BE DONE 4.0 START ***/
				if (strcmp(option_name, "If-Modified-Since") == 0 ) {
					...
					if(strptime(strtok_r(strtokr_save,"\r\n",&strtokr_save)," %a, %d %b %Y %T GMT",&since_tm)==NULL)
