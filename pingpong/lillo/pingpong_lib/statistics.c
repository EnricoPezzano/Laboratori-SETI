...
/*** TO BE DONE START ***/
	...
	struct timespec result; //Struct che indica secondi e nanosecondi
 
		if((last->tv_nsec - previous->tv_nsec) < 0)
		{
			result.tv_sec = last->tv_sec - previous->tv_sec -1 ;
			result.tv_nsec = last->tv_nsec - previous->tv_nsec + 1000000000 ;
		} 
		else
		{
		...
		return ((result.tv_sec * 1000.0)+ (result.tv_nsec / 1.0e6));
